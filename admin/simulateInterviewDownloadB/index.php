  ...
  if ( !isset( $_SESSION['name'] ) ) header( "Location: https://$_SERVER[SERVER_NAME]".dirname( $_SERVER['SCRIPT_NAME'] ).'/../' );
    ...
    while ($student = $studentsListQuery->fetch(PDO::FETCH_ASSOC)) {
      $sql =  "SELECT ".
              ...
              "FROM phase1 ".
              "LEFT JOIN TVERESchool ON TVERESchool.id = LEFT(phase1.depId,3) ".
              "LEFT JOIN TVEREDepartment ON TVEREDepartment.id = phase1.depId ".
              "WHERE phase1.id = :stuid AND phase1.pass ".
      ...
      if (!$statement->execute()) {
        ...
        $column = 'G';
        $colCounter = 0;
        while ($targets = $statement->fetch(PDO::FETCH_ASSOC)) $sheet->setCellValue(strval($column++.$rowCounter), $targets['depIdTitle']);
      }
    }
  
    // 準備下載檔案
    $fileName = '專業問題模擬面試名單(一階通過)_'.date('Ymd_His').'.xlsx';
